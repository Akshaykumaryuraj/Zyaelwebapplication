...
$(document).ready(function () {
    var datatable = $('#VendorsCredentialGrid').dataTable();
    datatable.fnDestroy();
...
function BindVendorsCredentialGrid(vendor) {
    var count = 1;
    var table = $("#VendorsCredentialGrid").DataTable({
        "processing": true,
        "serverSide": true,
        "filter": true,
        "searchable": true,
        ...
        "ajax": {
            ...
            data: function () {
                debugger
                var info = $('#VendorsCredentialGrid').DataTable().page.info();
                var TemplateStr = 'pageNumber=' + (info.page + 1);

                if ($('#VendorsCredentialGrid_length select').val() > 0) {
                    TemplateStr += '&pagesize=' + $('#VendorsCredentialGrid_length select').val();
                } else {
                    TemplateStr += '&pageSize=100';
                }
                TemplateStr += '&vendor=' + vendor;
                $('#VendorsCredentialGrid').DataTable().ajax.url(
